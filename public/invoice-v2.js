// #452(대표 GO 9/18) 송장변환 v2(/invoice-v2.html) — #452-p부터 회사프로그램 「송장변환」 메뉴 안에 iframe(?embed=1)으로 끼워 정식 사용. app.js의 변환·집계 함수는 무접촉(#463 9/21: 구버전 화면 마크업·화면 코드는 제거 — 「detectSize~채널 초기화 표식」·「qtyAggregated~resetInvoiceQty」 두 구간만 남아 이 파일이 잘라 쓴다).
