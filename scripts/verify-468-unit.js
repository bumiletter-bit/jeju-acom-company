...
(async () => {
    ...
    S.casePages['2026-09-22'] = [[order('A1', 50000), order('A2', 30000), order('B0', 20000), order('R1', -3000, 'PURCHASE_REVIEW')]];
    ...
    ok('① 본문 = 넣은 값·취소이월·리뷰적립·반품케어·유입·집화 대기', /넣은 값 80,100/.test(S.notes[0].msg) && /취소·이월 차이 100/.test(S.notes[0].msg) && /리뷰적립 −3,000/.test(S.notes[0].msg) && /반품케어 −500/.test(S.notes[0].msg) && /유입 \+20,000\(1건\)/.test(S.notes[0].msg) && /집화 대기 1건/.test(S.notes[0].msg), S.notes[0].msg);
    ...
    reset(); S.lms = [{ key: 'A1', ship: '2026-09-17', status: 'sent' }]; S.casePages['2026-09-18'] = [[order('A1', 1000000)]]; S.ss = [{ date: '2026-09-17', s: 1030000, u: 0 }];
    const r3 = await R.settleReconRun(daily({ expect: '2026-09-18', bs: '2026-09-17', be: '2026-09-17', pay: 1100000, fee: 100000 }));
    ok('③ 3% 차이 = warn · 본문에 0.5% 안내', r3.status === 'warn' && r3.diff1 === 30000 && /0\.5%를 넘습니다/.test(S.notes[0].msg) && /⚠️/.test(S.notes[0].title));
    ...
    S.ss = [{ date: '2026-09-18', s: 400500, u: 0 }, { date: '2026-09-20', s: 0, u: 600000 }, { date: '2026-09-17', s: 999999, u: 0 }];
    const r5 = await R.settleReconRun(daily({ expect: '2026-09-21', bs: '2026-09-18', be: '2026-09-20', pay: 1100000, fee: 100000 }));
    ok('⑤ 묶음(9/18~20) = 넣은 값 두 날짜 합 1,000,500(9/17 제외) · 기간 내 2건 · ok · 본문 「9/18~9/20 발송분」', r5.input_sum === 1000500 && r5.input_dates.length === 2 && r5.in_period_count === 2 && r5.status === 'ok' && /9\/18~9\/20 발송분/.test(S.notes[0].msg), S.notes[0].msg.slice(0, 80));
    const bnd = R.settleReconStatus({ input_sum: 1000000, in_period: 995000, complete_date: '2026-09-21', expect_date: '2026-09-21', todayKst: '2026-09-24' });
    const bnd2 = R.settleReconStatus({ input_sum: 1000000, in_period: 994999, complete_date: '2026-09-21', expect_date: '2026-09-21', todayKst: '2026-09-24' });
    ok('⑥ 경계 = 정확히 0.5%는 ok · 초과는 warn', bnd.status === 'ok' && bnd2.status === 'warn');
