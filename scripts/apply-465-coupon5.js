// #465(대표 9/21 "룰렛 실물 당첨 쿠폰 5% · 회원 3575099520@n 미지급 1건 처리 진행해"): 5% 룰렛 쿠폰 발급 — 재사용판
//   사용: node scripts/apply-465-coupon5.js <회원ID> <reward_grants id>
//   #433 스크립트와 다른 점: 「이미 보유면 발급 생략」이 아니라 **보유 장수 기준**(발급 전 n장 → 발급 후 n+1장) — 같은 회원이 두 번째 당첨된 경우(9/12 1장 보유 중 9/21 재당첨)에도 1장을 더 준다.
//   안전장치: 당첨 행이 pending·coupon5·그 회원인지 선검사 · 정의는 고정 번호(중복 생성 없음) · 발급은 회원 1명(M)만 · 장수가 +1이 아니면 지급완료 표시 안 함.
require('dotenv').config();
const { Pool } = require('pg');
const pool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: { rejectUnauthorized: false } });
const COUPON_NO = '6086230051600000967';   // 「5% 할인쿠폰(룰렛)」 발급일부터 30일 (#433 · 2026-09-12 생성)
const MEMBER = process.argv[2], GRANT_ID = Number(process.argv[3]);
const ACTOR = '클코(대표 지시 #465 룰렛 5% 쿠폰 지급)';
if (!MEMBER || !GRANT_ID) { console.error('사용: node scripts/apply-465-coupon5.js <회원ID> <reward_grants id>'); process.exit(1); }
...
(async () => {
    ...
    if (!def || def.deleted === 'T' || def.available_period_type !== 'R' || String(def.benefit_percentage) !== '5.0') throw new Error('쿠폰 정의 이상 — 중단: ' + JSON.stringify(def || null).slice(0, 200));
    ...
    await pool.query(`INSERT INTO audit_logs (action, target_type, target_id, changes, source, actor_id, actor_name) VALUES ('update','reward_grant',$1,$2,'reward-grant',NULL,$3)`,
        [GRANT_ID, JSON.stringify({ before: { status: 'pending' }, after: { status: 'granted', member_key: MEMBER, kind: 'coupon5', coupon_no: COUPON_NO, issue_no: got.issue_no }, note: '#465 API 발급(정의 재사용 · 발급일부터 30일 · 보유 ' + b.length + '→' + a.length + '장)' }), ACTOR]);
