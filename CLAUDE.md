...
## ★ 현재 상태 (2026-07-26 세션 종료 시점) — 다음 작업: STEP E 상품문의 자동답변

- **배포**: v5.9.96까지 라이브(app.js v=293). 7/26 하루 배포분 = 카페24 3채널 완성(v5.9.73)·조회기간 통일+무응답 탭(v5.9.80)·알림 설정(v5.9.84)·송장변환 직원 개방+진행 표시(v5.9.87)·카페24 토큰 KST 버그 수정+알림 문구 편집(v5.9.88)·AGENT OFFICE 직원 공유+자동 로그아웃+퇴사 처리(v5.9.90~91)·저장/완료 피드백(v5.9.92~93)·보고서 질문 표시(v5.9.94)·확인함 보고서 바로 열람(v5.9.95)·% 진행바(v5.9.96). 상세는 CHANGELOG.md.
- **백업 체계 완성**: ①배포 시 백업(강제) ②일일 백업 매일 07:00(작업 스케줄러 "제주아꼼이네 DB 일일백업" — 구 주간작업의 경로 따옴표 버그 수정·교체, StartWhenAvailable) ③**로테이션 B정책 가동**(오늘 전부·30일 내 하루 1개·이전 주 1개, 안전검증 내장, 일일 백업에 자동 연결 — 첫 정리 85개·2.3GB 완료) ④복구 절차서 `docs/DB복구절차.md`. ⚠️ .cmd 파일에 한글 주석 금지(CP949 인코딩 깨짐 — ASCII만).
- **STEP E — 상품문의(Q&A) 자동답변: v5.9.98 배포 (2026-07-27) — 시나리오 기반 생성(대표 검수·재시뮬레이션 합격)**. v5.9.97의 "배정(복사)"을 **생성**으로 변경: AI가 시나리오를 재료로 질문 맥락에 맞게 작성(1개면 각색·복수면 조합), **철칙 = 재료에 없는 사실 생성 금지·부족하면 SKIP**, 꼬리 문구 서버 강제, 상품 맥락([상품: OO]) 전달+가격표 품목 필터, 사용 시나리오명 복수 기록(`ai_scenarios`). 공개/비공개 구분은 **API 미제공 확인** → 전 답변 공개 기준 무차등(대표 승인). 톡톡봇도 동일 생성 방식(ai-handler.js — 되묻기 유도·신뢰 멘트·옵션 전환 원칙 포함, **push는 대표 직접**). 신규 시나리오 51 '맛 보증·무료수거 반품'·52 '로얄과·사이즈별 맛 차이' 등록(공통). 모델 claude-sonnet-5(env `QNA_MODEL`). 설계 v2(자동 게시) 대표 승인·스펙 `docs/superpowers/specs/2026-07-26-naver-qna-autoreply-design.md`. 구조: qna 수집(30분·기본 OFF) → 배정(키워드 1차+애매하면 LLM 번호/SKIP만·확정 문구만) → 확신 건 자동 게시(`qna_auto_post` 스위치 기본 ON·대표 전용) → SKIP은 [문의 관리 > 🛰️ 상품문의] 탭에서 직원 게시. 쓰기는 `naverPostQnaAnswer()` 1개 함수만(PII 차단 내장 — 리허설: 시나리오 51건 오차단 0·의도 PII 6/6 차단). **잔여 절차: ①대표 SSH install.sh 재실행(ALLOW GET+PUT 2줄, RELAY_VERSION 2026-07-27.1 확인) ②시나리오 1건만 '상품문의' 채널 등록 ③qna 수집 ON → 자동 게시 1건 판매자센터 실물 확인(수정란 포함) ④통과 후 시나리오 확장(공통 전환 ~25건·신규 8건 후보는 보고서 참조)**. 기존 51건 시나리오 전부 '톡톡' 채널 — '공통' 전환해도 봇 무영향(봇 조회 API가 톡톡+공통 수신).
- 대표 확인 잔여 2건: Render 워크스페이스 플랜(DB 복구창 3일/7일 판별)·OneDrive/Render 2단계 인증 설정(대표가 직접 하기로 함).
