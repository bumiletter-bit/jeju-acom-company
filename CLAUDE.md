...
> **★ 현재 상태 (7/29 지시 #90~#96 처리 — v5.9.137 라이브)**: **알림톡 4장 체제 대표 확정(A·B·D·E — #90 3장 → #93·94 E 신설 → #96 버튼 확정)**. C(배송완료) 폐기 유지(`docs/백로그_배송완료_트리거조사.md`). **철칙 변경(#92)**: 알림톡 가동 시 발송 성공 건만 네이버 발주확인 자동(스위치 OFF·dry-run 실측 합격). **발송 안내 = 알림톡 E 우선→SMS 대체(#93, LMS 단독 번복)** — E는 버튼식(#94: WL→공개 안내 페이지 `/guide?p=상품id` 라이브·19품목·크로스셀). **🔴 알리고 = 등록 IP만 허용(#91 실측 — #67 "IP 제한 없음" 판단 정정) → 전 호출 NCP 릴레이(101.79.16.213) 경유 확정(#95)**, 알리고에 IP 등록 완료(대표). 문안 단일 소스 JSON status='confirmed'·차단 2겹 유지. 실발송·등록·검수 신청 0건.
> **대표 손작업 대기**: ①**릴레이 갱신 1회(SSH)** — `curl -fsSL https://raw.githubusercontent.com/bumiletter-bit/jeju-acom-company/main/relay/install.sh | sudo bash` (RELAY_VERSION 2026-07-29.1 = /aligo 중계 + 발주확인 confirm ALLOW 동시 반영·env 보존) → 클코가 재테스트 → 통과 시 "심사 신청 준비 완료" ②E의 채널추가(AC) 병행 여부 ③카페24 scope 재동의 묶음 ④등록→검수 신청 대표 최종 GO ⑤D 마이페이지 '가입 혜택' 실존 확인. 상세 = 지시함 #92~#96 응답.
