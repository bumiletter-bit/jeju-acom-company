...
> - **#100 이미지형 알림톡 (진행중 — 대표 게이트 2건 대기)**: v5.9.140 준비 완료 — templates_image 4장(전 장 [문의하기] 버튼→pf.kakao.com/_lbJLs/chat·tpl_emtype=IMAGE·E는 4버튼)·릴레이 /aligo multipart(RELAY_VERSION **2026-07-29.2 — 대표 SSH 갱신 필요**)·등록 실행기 set='image'. **게이트: ①대표가 이미지를 `assets/alimtalk_image.jpg`에 저장(README 안내 있음·800×400·≤500KB — 리포 PUBLIC 고지) ②릴레이 SSH 한 줄 재실행** → 충족 시 클코가 규격 실측→배포→`aligo_register_request` {go:'yes',audit:true,set:'image'}로 등록·검수 신청.
> - **텍스트형 4장 심사 추적 중**: UJ_9084~9087(+구본 9082) 전부 R/REQ(7/30 01:41 최종 확인 — 폴링 턴마다 영업시간대 재확인, selftest의 senderkey_template_list.templates 필드). 승인 시 이미지형이 본판·텍스트형은 폴백(#100 전략).
