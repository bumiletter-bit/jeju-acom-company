...
- ⏳ **#238**(주문내역·알림 데모=완성 기준): 방식 판단 기록 완료 — **(A) 카페24 myshop 스킨링 채택**(skin6 원본 myshop/order/list.html에 Myshop_OrderHistory 모듈·치환코드 실재 확인. (B) 서버 API는 회원 인증 부재로 PII 위험 — 탈락). **다음 작업 = /myshop/order/list.html v5 카드 스킨링**. 알림 실이벤트화는 4단계와 병합.
