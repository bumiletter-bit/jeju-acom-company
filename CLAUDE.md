...
> **★ 현재 상태 (7/29 재부팅 후 새 세션, 지시 #90 처리)**: **알림톡 3장 체제 대표 확정(#90, 2026-07-29)** — 심사 대상 = A(주문완료-일반)·B(주문완료-예약)·D(회원가입, #88 수정안 채택·자사몰 가입자 한정). **C(배송완료)는 폐기**(구매확정 시점도 불채택 — 발송 시점 LMS가 역할 수행. 트리거 조사는 `docs/백로그_배송완료_트리거조사.md` 보존). 문안 단일 소스 `scripts/alimtalk-templates.json` status='confirmed', 실행 차단은 키 미투입·GO 2겹 유지. 게임 백엔드 v5.9.134 잠금(MALL_API=off) 유지. 실발송 0건·검수 신청 0건.
> **대표 확인 잔여**: ①카페24 scope 재동의 묶음(read_customer+write_promotion+write_mileage 1회 합산) ②알리고 키 투입→등록→검수 신청 대표 최종 GO ③D 등록 전 마이페이지 실존 '가입 혜택' 확인(없으면 해당 줄 통삭). 상세 = 지시함 #88·#90 응답.
