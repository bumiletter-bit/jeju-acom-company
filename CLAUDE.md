...
- 🏗️ **#236(#232 승인 — 4단계 시공)**: ✅1단계 회원 연동(probe `#akmMemberSrc`+`window.AKM_MEMBER`·헤더↔마이 단일 소스 — 비로그인+미설정=시각 무변경. 카페24 실측: 비로그인=statelogoff 블록만 렌더) ✅2단계 주문내역(로그인→/myshop 이동)·알림종(로그인=정직 빈 상태) ✅3단계 챗 배선(`CHAT_OPEN=false` 단일 스위치 — true 한 줄로 입력창 disabled 해제+실API. OFF=호출 0 실증) ⏳**4단계 게임 미착수**(MALL_API env 대표 클릭 대기)
