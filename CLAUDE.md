...
- **★ 알림톡 자동화 프로젝트 가동(7/28, 지시함 파이프라인)**: #67 사전 조사 3종 완료(알리고 API·카페24 scope·재구매 데이터 — 지시함 응답 참조) → **#68 기반 시공 C2~C6 완료·배포(v5.9.128)**: 품목별 안내문(bot_products.notify_message)·발송일 계산(shipping-schedule.js — ⚠️ 공휴일·규칙 대표 검수 대기)·알림톡 뼈대(kakao-notify.js + 수집기 kakao_notify **기본 OFF**·이력 kakao_notify_log·실발송 이중 차단 env KAKAO_NOTIFY+ALIGO_* 4종)·시즌 대기 신청(season_waitlist + 판매현황 탭 하단)·텔레그램 kakaosend 알림(기본 OFF). **실발송 가동 전 대표 손작업**: 발신프로필 등록→템플릿 검수(4~5일)→포인트 충전→C3 규칙 검수→env 입력+타이머 ON (상세는 지시 #68 응답). 지시함 폴링은 `cc_instructions` 직접 UPDATE 방식(respond 스크립트 부재 — 세션 스크래치에서 수행).
