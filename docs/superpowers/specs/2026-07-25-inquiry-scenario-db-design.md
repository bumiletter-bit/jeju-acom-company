...
## 7-b. 다음 프로젝트(②③) 확정값 메모 — 대표 확정 2026-07-25

- **문의 수집 타이머: 30분 간격** (가장 짧은 타이머 — 자동답변도 즉시일 필요 없음)
- **정산 수집: 하루 2회 고정 시각** — 아침 9:30~10시경 1회 + **오후 7~8시경 1회**(당일정산이 오후에 뜨므로). 시각·주기는 화면에서 변경 가능(하드코딩 금지, `naver_auto_collect` 확장).
- 전부 기본 OFF 시작, 429 대비 350ms 간격+지수 백오프 패턴 재사용 (CLAUDE.md [B] 규칙 유지).
