...
async function initDB() {
    ...
    if (itemCount.rows[0].c === 0) {
        const ITEM_SEED = [
            ...
            "과즙팡팡 황금향 / 상품 및 과수: 황금향 선물용 - 3kg(중대과 7~15과)",   // 지시 #428(9/7): 네이버 옵션명 변경 반영(시드는 빈 테이블일 때만 — 실DB items 10·11은 직접 갱신)
            "과즙팡팡 황금향 / 상품 및 과수: 황금향 선물용 - 5kg(중대과 13~25과)",
    ...
    if (skCnt.rows[0].n === 0) {
        const SK_SEED = [
            ...
            ['하우스감귤', '초기(푸른끼)', '05-01', '07-31', '푸른끼가 많은 시기예요 — 푸른끼가 있어도 다 익은 것이니 안심하시라고 안내해주세요(강제 착색 없이 신선하게 발송하는 것이 장점). 맛은 새콤달콤해요. 과즙이 많아 냉장 보관 후 시원하게 드시면 새콤달콤함이 더 살아나요.', 1],   // #437(9/14): 계절 단어 제거(실DB id 11 동일)
            ['하우스감귤', '후기(단맛)', '08-01', '10-15', '단맛이 우세해지고 겉색이 일반 귤색이 되는 시기예요(맛이 더 올라옴). 시설재배 물조절로 당도를 높인 것이 하우스귤의 장점이에요. 하우스귤은 과즙이 많아 냉장 보관 후 시원하게 드시면 단맛이 더 살아나요. (새콤하게 느껴지면 상온에서 하루 이틀 두었다가 드셔도 좋아요.)', 2],   // #437(9/14): 계절 단어 제거(실DB id 12 동일)
        ];
...
async function telegramAlertTemplates() {
    ...
    try {
        const r = await pool.query(`SELECT value FROM agent_office_config WHERE key = 'telegram_alert_templates'`);
        const v = (r.rows[0] && r.rows[0].value) || {};
        for (const k of TELEGRAM_ALERT_KEYS) if (typeof v[k] === 'string' && v[k].trim()) out[k] = v[k];
    } catch (_) { /* 조회 실패 → 기본 문구 */ }
    return out;
}
async function alertText(kind, vars) {
    const tpl = (await telegramAlertTemplates())[kind] || '';
    return tpl.replace(/\{\{(.+?)\}\}/g, (m, k) => (k in vars ? String(vars[k]) : m));
}
...
app.put('/api/agent-office/naver/alert-settings', authMiddleware, adminOnly, async (req, res) => {
    try {
        ...
        // 문구 템플릿 저장 (선택): 빈 문자열 = 기본 문구로 복원(오버라이드 삭제). 500자 제한.
        if (req.body && typeof req.body.templates === 'object' && req.body.templates) {
            const saved = await pool.query(`SELECT value FROM agent_office_config WHERE key = 'telegram_alert_templates'`);
            const tpl = { ...((saved.rows[0] && saved.rows[0].value) || {}) };
            for (const k of TELEGRAM_ALERT_KEYS) {
                if (typeof req.body.templates[k] !== 'string') continue;
                const v = req.body.templates[k].trim().slice(0, 500);
                if (v) tpl[k] = v; else delete tpl[k];
            }
...
app.get('/api/agent-office/naver/qnas', authMiddleware, async (req, res) => {
    try {
        const r = await pool.query(
            `SELECT question_id, raw, answered, ai_status, ai_scenario_id, ai_scenarios, ai_draft, posted_at, posted_by, post_error, collected_at
        ...
        const scen = await pool.query(`SELECT id, name FROM inquiry_scenarios`);   // 구버전 기록(ai_scenario_id) 표기용
        const names = Object.fromEntries(scen.rows.map(s => [s.id, s.name]));
        res.json({ ok: true, auto_post: auto, rows: r.rows.map(x => ({ ...x,
            scenario_name: (Array.isArray(x.ai_scenarios) && x.ai_scenarios.length)
...
// 판매현황·가격표 데이터 (톡톡봇 products-store.js와 동일 규칙 — 그룹·용량순, 준비중 미노출)
async function qnaStoreData() {
...
지금부터 네이버 스마트스토어 "상품문의(Q&A)" — 모든 사람이 볼 수 있는 공개 게시판 — 에 게시될 답변을 작성합니다.
...
- "사진을 보내달라"·"주소/성함을 남겨달라"처럼 게시판에 개인정보·자료를 올리게 하는 표현은 금지 — 톡톡·고객센터로 유도하세요.

## SKIP — 아래 경우에는 답변을 작성하지 말고 SKIP만 출력
- 재료로 답할 수 없는 문의 (재료에 없는 상품·사실이 필요한 경우)
- 어떤 상품이 고객 취향에 더 맞을지 개인 판단·추천을 요구하는 문의 (재료의 사실 안내만으로 충분히 답이 되는 경우는 답변 가능)
- 광고·스팸·협찬 제안·업체 홍보 글
- 의미를 파악할 수 없는 문의
- 조금이라도 애매하면 SKIP이 정답입니다.

## 출력 형식 (정확히 지키세요)
- 답변하는 경우: 첫 줄에 "사용시나리오: 이름1 | 이름2" (재료로 쓴 시나리오 이름 전부, 재료 목록의 이름 그대로), 둘째 줄부터 답변 본문만.
...
async function collectQna() {
    ...
    // 신규 미답변 → 시나리오 기반 생성 → 자동 게시(스위치 ON일 때만). 실패·SKIP·스위치 OFF = 직원 답변 대기.
    ...
    if (fresh.length) {
        ...
        for (const q of fresh) {
            ...
            const draft = gen.answer;
            await pool.query(`UPDATE naver_qnas SET ai_status='draft', ai_scenarios=$2, ai_draft=$3 WHERE question_id=$1`,
                [q.id, JSON.stringify(gen.used), draft]);
...
// #427(대표 GO 9/6): 옵션 이름 → 비교 키(과일+용도+중량+등급). 프리픽스·(특가)·(제철)·행사문구를 무시하고 실옵션만 추출.
//   카페24(「·」)·네이버(「/」) 형식이 달라도 같은 옵션이면 같은 키가 나온다. 한쪽에만 있는 옵션(못난이 등)은 매칭 안 되어 비교에서 자연 제외.
function cafe24OptKey(s) {
...
async function collectCafe24Sync(modeOverride, opt) {
    ...
    // #427(대표 GO 9/6): 옵션별 결제가 감지 — 기본가만 자동 동기화되는 한계 보완(감지형·읽기 전용·비파괴).
    ...
    try {
        for (const [nno, m] of entries) {
            const sn = snapBy[nno], cp = curBy[String(m.c24)];
            if (!sn || !cp || cp.display !== 'T') continue;
            const nOpts = {};
            for (const o of (sn.opts || [])) {
                if (o.usable === false) continue;
                const k = cafe24OptKey((o.n1 || '') + ' ' + (o.n2 || ''));
                if (k) nOpts[k] = Number(sn.discPrice) + Number(o.price || 0);
            }
            if (!Object.keys(nOpts).length) continue;   // 옵션 미수집 상품 건너뜀(낡은 데이터로 오경보 방지)
            let vr;
        ...
        if (req.action === 'status') {
            ...
            try {
                ...
                const okPath = /^\/api\/v2\/admin\/products/.test(rp) || /^\/api\/v2\/admin\/categories\/\d+\/products$/.test(rp) || /^\/api\/v2\/admin\/mains/.test(rp)
                    ...
                    // #436(대표 9/13 "m.akkome.com 옛 모바일샵 — 니가 직접 정리"): 모바일샵 설정 조회/변경(PUT은 use_mobile_page 1필드만 — 공식 문서 「Mobile setting」 실브라우저 확인)
                    || (/^\/api\/v2\/admin\/mobile\/setting$/.test(rp) && (method === 'GET'
                        || (method === 'PUT' && req.body && req.body.request && Object.keys(req.body.request).every(k => k === 'use_mobile_page') && /^[TF]$/.test(String(req.body.request.use_mobile_page)))))
